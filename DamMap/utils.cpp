...
//Fast version of root function.
//Borrowed from http://supp.iar.com/FilesPublic/SUPPORT/000419/AN-G-002.pdf
...
//Get distance from x and y coordinates
int distance(int x0, int y0, int x1, int y1) {
	return root((x1 - x0)*(x1 - x0) + (y1 - y0)*(y1 - y0));
}

//Get angle of a line stored as a cv::Vec4i (vector of four integers)
double angleOfLine(Vec4i line) {
	if ((line[2] - line[0]) == 0) return CV_PI / 2;
	return atan((line[3] - line[1]) / (line[2] - line[0]));
}

//Compare the lengths of two lines. Used for sorting by length
...
//Returns a maxumum value of a sample of a specified channel in a region of interest
int getAvgChannelNearby(const Mat& roi, int channel, int iterations) {
	int sum = 0;
	for (int i = 0; i < iterations; i++) { //Iterations is sample size, adjusted based on time constraint
...
//Returns the maximum value of a pseudorandom sample of a specified channel in a region of interest
int getMaxChannelNearby(const Mat& roi, int channel, int iterations) {
	int max = 0;
	for (int i = 0; i < iterations; i++) { //Iterations is sample size, adjusted based on time constraint
		int col = (i * 17) % roi.cols; //Get pseudorandom column
		int row = (i * 41) % roi.rows; //Get pseudorandom row
...
//Determines if an angle is closer to being vertical than horizontal.
bool moreVerticalThanHorizontal(double angle) {
	return abs(abs(angle) - CV_PI / 2) < CV_PI / 4;
}

//Returns a cropped and scaled portion of a quadrant of an image so that it can be processed
Mat getQuadrant(Mat& img, int quadrant) {
	Mat roi; //Region of interest
	if (quadrant == 1) {
		roi = img(Range(0, img.rows / 2), Range(img.cols / 2, img.cols - 1)); //Region of interest is quadrant 1
	}
	else if (quadrant == 2) {
		roi = img(Range(0, img.rows / 2), Range(0, img.cols / 2));  //Region of interest is quadrant 2
	}
	else if (quadrant == 3) {
		roi = img(Range(img.rows / 2, img.rows - 1), Range(0, img.cols / 2)); //Region of interest is quadrant 3
	}
	else {
		roi = img(Range(img.rows / 2, img.rows - 1), Range(img.cols / 2, img.cols - 1)); //Region of interest is quadrant 4
	}
	roi = roi(Range(roi.rows / 7, roi.rows * 6 / 7), Range(roi.cols / 7, roi.cols * 3 / 5)); //Crop image
	return roi;
}
